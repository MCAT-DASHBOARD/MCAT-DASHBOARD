...
def run_pipeline(local_mode=False):
    ...
    try:
        ...
        btc_dpo = {"value": btc_dpo_val, "in_buy_zone": btc_dpo_val is not None and btc_dpo_val < 20}
        print(f"    BTC: 1D DPO = {btc_dpo_val:.1f} {'← BUY ZONE' if btc_dpo['in_buy_zone'] else ''}")
    except Exception as e:
        print(f"    ⚠️ BTC 1D DPO failed: {e}")
        btc_dpo = {"value": None, "in_buy_zone": False, "error": str(e)}
