...
# ── Dependencies ──────────────────────────────────────────────
# pip install yfinance requests scipy
...
def calc_dpo_percentile_series(dpo_raw_array):
    """
    Percentile rank normalization for a raw DPO array.
    For each value, computes: what percentage of all prior observations were <= this value.
    Returns numpy array on 0-100 scale. Expanding window (uses all history up to current point).
    Minimum 10 observations before producing output.

    This is the entry/exit normalization — used for signal detection (PC<12)
    and two-stage exit (OH>90, TP<60).

    Uses scipy.stats.percentileofscore with kind='rank'. Numpy-only input/output.
    """
    n = len(dpo_raw_array)
    ...
    for i in range(n):
        ...
        if len(vals) < 10:
            continue
        arr = np.array(vals)
        result[i] = scipy_stats.percentileofscore(arr, v, kind='rank')
    return result


def calc_raw_dpo_array(prices_list, period=7):
    """
    Compute raw (unnormalized) DPO array for a given period.
    DPO = Close - SMA(period, shifted back period/2+1 bars)
    Returns numpy array with NaN for insufficient data points.
    """
...
def fetch_1d_dpo(cg_id, ticker_label="", retries=1):
    """
    Fetch 250 days of price history from CoinGecko and compute DPOs.
    V6: Returns both min/max final values AND raw DPO arrays for percentile rank.
    ...
    for attempt in range(retries + 1):
        try:
            ...
            if len(prices) < 70:
                ...
                return {"value": None, "dpo_1w": None, "dpo_2w": None, "hurst": None, "in_buy_zone": False,
                        "raw_dpo7": None, "raw_dpo20": None, "raw_dpo40": None, "prices": None,
                        "error": f"insufficient data ({len(prices)} points)"}

            # Min/max normalization (existing)
            ...
            hurst_val = compute_hurst(prices, window=90)

            # V6 C2: Compute raw DPO arrays for percentile rank (computed in main loop)
            raw_dpo7 = calc_raw_dpo_array(prices, period=7)
            raw_dpo20 = calc_raw_dpo_array(prices, period=20)
            raw_dpo40 = calc_raw_dpo_array(prices, period=40)

            if dpo_val is None:
                print(f"    ⚠️ {label}: DPO computation returned None")
                return {"value": None, "dpo_1w": None, "dpo_2w": None, "hurst": None, "in_buy_zone": False,
                        "raw_dpo7": None, "raw_dpo20": None, "raw_dpo40": None, "prices": None,
                        "error": "DPO computation failed"}

            in_buy = dpo_val < 20
            h_str = f", H={hurst_val:.3f}" if hurst_val is not None else ""
            print(f"    {label}: 1D DPO = {dpo_val:.1f}, 1W = {dpo_1w}, 2W = {dpo_2w}{h_str} {'← BUY ZONE' if in_buy else ''}")
            return {"value": dpo_val, "dpo_1w": dpo_1w, "dpo_2w": dpo_2w, "hurst": hurst_val, "in_buy_zone": in_buy,
                    "raw_dpo7": raw_dpo7, "raw_dpo20": raw_dpo20, "raw_dpo40": raw_dpo40, "prices": prices}
...
def compute_hurst(prices, window=90):
    """
    Compute Hurst exponent using Rescaled Range (R/S) method.
    H < 0.5 = mean-reverting (DPO works well), H > 0.5 = trending (DPO less reliable).
    """
...
def classify_hurst(H):
    """
    Classify Hurst into regime bands.
    Returns: (regime_label, confidence_multiplier)
    """
    if H is None:
        return 'UNKNOWN', 0.75
    if H < 0.45:
        return 'STRONG_REVERSION', 1.0
    elif H < 0.50:
        return 'REVERSION', 1.0
    elif H < 0.55:
        return 'NEUTRAL', 0.75
    elif H < 0.60:
        return 'MILD_TREND', 0.5
    else:
        return 'STRONG_TREND', 0.25


# ═══════════════════════════════════════════════════════════════
#  TREND COMPUTATION (V6 C7)
# ═══════════════════════════════════════════════════════════════

def compute_weekly_rsi(prices_daily, period=14):
    """
    Compute 14-period RSI on weekly-resampled prices.
    Returns the most recent weekly RSI value (float 0-100), or None if insufficient data.
    RSI measures momentum exhaustion — independent from DPO's structural deviation.
    No new dependencies — pure Python math.
    """
    if len(prices_daily) < (period + 1) * 7:
        return None

    # Resample to weekly (take last value per 7-day block)
    weekly_prices = []
    for i in range(0, len(prices_daily) - 6, 7):
        weekly_prices.append(prices_daily[min(i + 6, len(prices_daily) - 1)])
    ...
    for i in range(1, len(weekly_prices)):
        changes.append(weekly_prices[i] - weekly_prices[i - 1])

    # Initial average gain/loss over first N periods
    gains = [max(c, 0) for c in changes[:period]]
    losses = [abs(min(c, 0)) for c in changes[:period]]
    avg_gain = sum(gains) / period
    avg_loss = sum(losses) / period

    # Smoothed RSI (Wilder's method) for remaining periods
...
def compute_trend(dpo_pct_array, lookback=5):
    """
    Compute trend from last N percentile rank DPO values.
    Returns 'up', 'down', or 'flat'.
    """
    valid = dpo_pct_array[~np.isnan(dpo_pct_array)]
    if len(valid) < lookback:
        return 'flat'
    last_n = valid[-lookback:]
    delta = last_n[-1] - last_n[0]
    if delta > 3:
        return 'up'
    elif delta < -3:
        return 'down'
...
DEFAULT_STATE = {
    ...
    'entry_price': None,          # V6 C4: price at signal fire for stop-loss
    're_entry_state': 'ELIGIBLE',  # V6 C5: ELIGIBLE | BLOCKED | RESET_PENDING
}
...
def detect_signal(asset_state, dpo20_today, dpo40_today, current_price=None):
    """
    V6 C3: Signal detection using percentile rank normalization.
    Both DPO-20 and DPO-40 must be below 12th percentile for 2+ consecutive days.
    C4: Stores entry_price on new signal.
    C5: Respects re_entry_state gate.
    """
    THRESHOLD = 12  # V6: percentile rank threshold (was 20 for min/max)
    MIN_CLUSTER = 2
    COOLDOWN_DAYS = 30

    # V6 C5: Re-entry gate — blocked assets cannot fire new signals
    if asset_state.get('re_entry_state', 'ELIGIBLE') != 'ELIGIBLE':
        # Still track cluster_days for state consistency
    ...
    if both_below:
        ...
        if asset_state['cluster_days'] >= MIN_CLUSTER:
            ...
            if not asset_state.get('signal_active'):
                ...
                asset_state['entry_price'] = current_price  # V6 C4: Store for stop-loss
...
def update_cycle_phase(asset_state, dpo20_today, dpo40_today):
    ...
    V6: Thresholds updated for percentile rank normalization.
    """
    ...
    # V6: OH threshold 90 (was 80 for min/max)
    if d40 > 90 or d20 > 90:
        new_phase = 'OVERHEATED'
        was_oh = True

    # V6: TP threshold 60 (matches two-stage exit)
    ...
    # V6: ACCUMULATION threshold 12 (matches PC<12 entry)
    elif d20 < 12 and d40 < 12:
        new_phase = 'ACCUMULATION'

    # V6: EARLY_SIGNAL threshold 15 (proportional adjustment from 25)
    elif d20 < 15 or d40 < 15:
        new_phase = 'EARLY_SIGNAL'

    # RISING threshold kept at 30 — may need calibration for percentile rank
...
def update_exit_level(asset_state, dpo20_today, dpo40_today, symbol, current_price=None):
    """
    V6 C6: Two-stage exit framework using percentile rank normalization.

    Stage 1: OVERHEATED — EITHER DPO exceeds 90th percentile.
             This is a STATE TRANSITION, NOT an exit. Sets was_overheated = True.

    Stage 2: TAKE_PROFITS — BOTH DPOs cool below 60th percentile
             AFTER was_overheated is True. This IS the actual exit.

    Also: 20% stop-loss from entry price (C4).
    Also: 120-day stale timeout (handled in update_re_entry_state).
    """
    if not asset_state.get('signal_active'):
        asset_state['exit_level'] = None
        return asset_state
    ...
    # V6 C4: Stop-loss check — 20% below entry price
    entry_price = asset_state.get('entry_price')
    if entry_price and current_price and current_price <= entry_price * 0.80:
        asset_state['exit_level'] = 'STOPPED_OUT'
        asset_state['signal_active'] = False
        return asset_state
    ...
    # If overheated but not yet cooled — mark as overheated (informational, NOT an exit)
    if was_oh:
        asset_state['exit_level'] = 'OVERHEATED'  # Display only — signal stays active
        return asset_state
...
def update_re_entry_state(asset_state, dpo20_pct_today, dpo40_pct_today):
    """
    V6 C5: 3-state machine preventing cascade re-entries.
    ELIGIBLE → signal can fire
    BLOCKED → signal just exited, waiting for DPOs to rise above 12
    RESET_PENDING → DPOs have risen, waiting for them to drop below 12 again
    """
    THRESHOLD = 12
    state = asset_state.get('re_entry_state', 'ELIGIBLE')
    exit_level = asset_state.get('exit_level')

    # Detect exit → BLOCKED
    if exit_level in ('STOPPED_OUT', 'TAKE_PROFITS') and state == 'ELIGIBLE':
        state = 'BLOCKED'

    # Stale timeout → BLOCKED (120-day timeout)
    if asset_state.get('signal_active') and asset_state.get('signal_days_ago', 0) >= 120:
        asset_state['signal_active'] = False
        asset_state['exit_level'] = 'STALE'
        state = 'BLOCKED'

    # BLOCKED → RESET_PENDING when both DPOs rise above threshold
    if state == 'BLOCKED':
        d20 = dpo20_pct_today if dpo20_pct_today is not None else 50.0
        d40 = dpo40_pct_today if dpo40_pct_today is not None else 50.0
        if d20 >= THRESHOLD and d40 >= THRESHOLD:
            state = 'RESET_PENDING'

    # RESET_PENDING → ELIGIBLE when both DPOs drop below threshold
    elif state == 'RESET_PENDING':
        d20 = dpo20_pct_today if dpo20_pct_today is not None else 50.0
        d40 = dpo40_pct_today if dpo40_pct_today is not None else 50.0
        if d20 < THRESHOLD and d40 < THRESHOLD:
            state = 'ELIGIBLE'

    asset_state['re_entry_state'] = state
    return asset_state
...
def process_asset_automation(symbol, dpo7_minmax, dpo20_minmax, dpo40_minmax,
                             ...
                             trend_1w, trend_2w,
                             current_price=None, prev_state=None):
    """
    V6: Run full automation pipeline for one asset. ORDER MATTERS.
    Now receives BOTH normalizations. Uses percentile rank for logic,
    min/max for backward-compatible display fields.
    ...
    state['dpo_1w'] = round(dpo20_minmax, 1) if dpo20_minmax is not None else None
    state['dpo_2w'] = round(dpo40_minmax, 1) if dpo40_minmax is not None else None

    # V6 C8: Percentile rank values — new KV fields
    state['dpo7_pct'] = round(dpo7_pct, 1) if dpo7_pct is not None and not np.isnan(dpo7_pct) else None
    state['dpo20_pct'] = round(dpo20_pct, 1) if dpo20_pct is not None and not np.isnan(dpo20_pct) else None
    state['dpo40_pct'] = round(dpo40_pct, 1) if dpo40_pct is not None and not np.isnan(dpo40_pct) else None

    # V6 C7: Trend fields
    state['trend_1w'] = trend_1w
    state['trend_2w'] = trend_2w
...
def classify_mqs(vix_val, btc_above_200ma, spx_co_bottom, yen_unwind, m2_expanding, m2_contracting):
    ...
    # Rule 6: VIX <20 + BTC bear + no co-bottom → RED
    # V6 D2: demoted from AMBER — 12.5% hit rate, -13.8% avg in walk-forward
    elif vix < 20 and bear and not co_bot:
        tier, rule, name = "RED", 6, "VIX <20 + BTC bear (demoted v6)"
        ev, hit, n, sizing = 26.5, 75, 20, "20–30%"
...
def run_pipeline(local_mode=False):
    ...
    try:
        ...
        btc_hurst = compute_hurst(btc_prices, window=90)
        # V6 C2: Raw DPO arrays for percentile rank
        btc_raw7 = calc_raw_dpo_array(btc_prices, period=7)
        btc_raw20 = calc_raw_dpo_array(btc_prices, period=20)
        btc_raw40 = calc_raw_dpo_array(btc_prices, period=40)
        btc_dpo = {"value": btc_dpo_val, "dpo_1w": btc_dpo_1w, "dpo_2w": btc_dpo_2w, "hurst": btc_hurst,
                    "in_buy_zone": btc_dpo_val is not None and btc_dpo_val < 20,
                    "raw_dpo7": btc_raw7, "raw_dpo20": btc_raw20, "raw_dpo40": btc_raw40, "prices": btc_prices}
        ...
        btc_dpo = {"value": None, "dpo_1w": None, "dpo_2w": None, "hurst": None, "in_buy_zone": False,
                    "raw_dpo7": None, "raw_dpo20": None, "raw_dpo40": None, "prices": None, "error": str(e)}
    ...
    for ticker in CG_IDS:
        ...
        current_price_val = prices.get(ticker, {}).get("price")

        # V6 C2: Compute percentile rank from raw DPO arrays
        raw7 = dpo_data.get("raw_dpo7")
        raw20 = dpo_data.get("raw_dpo20")
        raw40 = dpo_data.get("raw_dpo40")

        dpo7_pct_today = None
        dpo20_pct_today = None
        dpo40_pct_today = None
        trend_1w_val = 'flat'
        trend_2w_val = 'flat'

        if raw20 is not None and raw40 is not None:
            pct7_series = calc_dpo_percentile_series(raw7) if raw7 is not None else np.array([])
            pct20_series = calc_dpo_percentile_series(raw20)
            pct40_series = calc_dpo_percentile_series(raw40)

            # Get today's percentile rank value (last non-NaN)
            valid7 = pct7_series[~np.isnan(pct7_series)]
            valid20 = pct20_series[~np.isnan(pct20_series)]
            valid40 = pct40_series[~np.isnan(pct40_series)]

            dpo7_pct_today = float(valid7[-1]) if len(valid7) > 0 else None
            dpo20_pct_today = float(valid20[-1]) if len(valid20) > 0 else None
            dpo40_pct_today = float(valid40[-1]) if len(valid40) > 0 else None

            # V6 C7: Compute trends from percentile rank series
            trend_1w_val = compute_trend(pct20_series)
        ...
        # FALLBACK: If fetch failed, carry forward yesterday's DPO values
        fetch_failed = (dpo7 is None)
        if fetch_failed and prev is not None:
            fallback_7 = prev.get('dpo_7')
            fallback_20 = prev.get('dpo_1w')
            fallback_40 = prev.get('dpo_2w')
            if fallback_7 is not None:
                dpo7 = fallback_7
                dpo20 = fallback_20
                dpo40 = fallback_40
                print(f"    {ticker}: Using yesterday's DPO values (fetch failed)")
            # Also carry forward percentile rank values on fetch failure
            if dpo20_pct_today is None and prev.get('dpo20_pct') is not None:
                dpo7_pct_today = prev.get('dpo7_pct')
                dpo20_pct_today = prev.get('dpo20_pct')
                dpo40_pct_today = prev.get('dpo40_pct')
                trend_1w_val = prev.get('trend_1w', 'flat')
                trend_2w_val = prev.get('trend_2w', 'flat')
                print(f"    {ticker}: Using yesterday's percentile rank values (fetch failed)")

        state = process_asset_automation(
            ticker, dpo7, dpo20, dpo40,
            dpo7_pct_today, dpo20_pct_today, dpo40_pct_today,
            trend_1w_val, trend_2w_val,
            current_price=current_price_val, prev_state=prev
        )

        # Track fetch status
        state['fetch_failed'] = fetch_failed
        if not fetch_failed:
            state['last_fresh_fetch'] = date.today().isoformat()
        else:
            state['last_fresh_fetch'] = prev.get('last_fresh_fetch') if prev else None

        # Hurst regime detection
        hurst_val = dpo_data.get("hurst")
        if hurst_val is None and prev is not None:
            hurst_val = prev.get('hurst_90')  # carry forward on fetch failure
        regime, h_conf = classify_hurst(hurst_val)
        state['hurst_90'] = hurst_val
        state['hurst_regime'] = regime
        state['hurst_confidence'] = h_conf

        # Weekly RSI — momentum exhaustion indicator (informational, not a gate)
        price_list = dpo_data.get("prices")
        if price_list and len(price_list) > 0:
            state['weekly_rsi'] = compute_weekly_rsi(price_list)
        elif prev is not None:
            state['weekly_rsi'] = prev.get('weekly_rsi')  # carry forward on fetch failure
        else:
            state['weekly_rsi'] = None
